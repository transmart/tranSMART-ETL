...
( trial_id 		character varying
 ...
 ,p_source_cd		character varying default 'STD'		--	default source_cd = 'STD'
...
BEGIN
	...
	logBase := log_base;
	sourceCd := upper(coalesce(p_source_cd,'STD'));
	...
	if pExists > 0 then
		...
		where trial_name = TrialId
		  and assay_id in
			  (select distinct x.assay_id from deapp.de_subject_sample_mapping x
			   where x.trial_name = TrialId
				 and coalesce(x.source_cd,'STD') = sourceCd
				 and x.platform = 'MRNA_AFFYMETRIX');
		get diagnostics rowCt := ROW_COUNT;
	...
	select distinct topNode || regexp_replace(replace(replace(replace(replace(replace(replace(category_cd,'+','\'),'PLATFORM',title),'ATTR1',coalesce(attribute_1,'')),'ATTR2',coalesce(attribute_2,'')),'TISSUETYPE',coalesce(tissue_type,'')),'_',' ') || '\','(\\){2,}', '\', 'g')
		  ,category_cd
	...
	select distinct topNode || regexp_replace(replace(replace(replace(replace(replace(
	       substr(replace(category_cd,'+','\'),1,tm_cz.instr(category_cd,'PLATFORM')+8),'PLATFORM',title),'ATTR1',coalesce(attribute_1,'')),'ATTR2',coalesce(attribute_2,'')),'TISSUETYPE',coalesce(tissue_type,'')),'_',' ') || '\',
		   '(\\){2,}', '\', 'g')
	...
	select distinct topNode || regexp_replace(replace(replace(replace(replace(replace(
	       substr(replace(category_cd,'+','\'),1,tm_cz.instr(category_cd,'ATTR1')+5),'PLATFORM',title),'ATTR1',coalesce(attribute_1,'')),'ATTR2',coalesce(attribute_2,'')),'TISSUETYPE',coalesce(tissue_type,'')),'_',' ') || '\',
		   '(\\){2,}', '\', 'g')
	...
	select distinct topNode || regexp_replace(replace(replace(replace(replace(replace(
	       substr(replace(category_cd,'+','\'),1,tm_cz.instr(category_cd,'ATTR2')+5),'PLATFORM',title),'ATTR1',coalesce(attribute_1,'')),'ATTR2',coalesce(attribute_2,'')),'TISSUETYPE',coalesce(tissue_type,'')),'_',' ') || '\',
		   '(\\){2,}', '\', 'g')
	...
	select distinct topNode || regexp_replace(replace(replace(replace(replace(replace(
	       substr(replace(category_cd,'+','\'),1,tm_cz.instr(category_cd,'TISSUETYPE')+10),'PLATFORM',title),'ATTR1',coalesce(attribute_1,'')),'ATTR2',coalesce(attribute_2,'')),'TISSUETYPE',coalesce(tissue_type,'')),'_',' ') || '\',
		   '(\\){2,}', '\', 'g')
	...
	loop
		...
		select tm_cz.i2b2_delete_1_node(r_Nodes.c_fullname,jobId) into rtnCd;
		stepCt := stepCt + 1;
	...
	from (select distinct b.patient_num as patient_id
			  ...
			  ,coalesce(a.source_cd,'STD') as source_cd
	...
	from (select site_id
		  ...
		  where trial_name = TrialId
			...
			and platform = 'MRNA_AFFYMETRIX') x;
...
*/
	...
	if dataType = 'T' then
		...
		select TrialId
			  ...
			  ,d.log_intensity
			  ,round(case when c.stddev_intensity = 0 then 0
	...
	/*	moved to Kettle after vacuum deapp.de_subject_microarray_data
...
SET search_path=tm_cz, tm_lz, tm_wz, deapp, i2b2demodata, pg_temp;
