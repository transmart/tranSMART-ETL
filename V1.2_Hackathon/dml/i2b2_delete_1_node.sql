CREATE OR REPLACE function tm_cz.i2b2_delete_1_node
(
...
BEGIN
	...
	stepCt := 0;
	stepCt := stepCt + 1;
	select tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Start i2b2_delete_1_node: ' || p_path,0,stepCt,'Done') into rtnCd;
	
	if coalesce(p_path,'') = ''  or p_path = '%' 
	then 
    ...
    where concept_cd in (select c_basecode from i2b2metadata.i2b2 where c_fullname = p_path);
	exception
	when others then
