CREATE OR REPLACE FUNCTION tm_cz.i2b2_load_chrom_region(input_file character varying, currentjobid numeric DEFAULT (-1))
  RETURNS numeric AS
$BODY$
...
BEGIN
	...
	-- Store data from our tab-delimeted input-file into a tempory table in the landing-zone
	...
	stepCt := stepCt + 1;
	select tm_cz.cz_write_audit(jobId,databaseName,procedureName,'Data copied from file ' || input_file,rowCt,stepCt,'Done') into rtnCd;
...
-- insert region definitions into deapp-schema
	...
	select  nextval('tm_cz.seq_region_id')
	        , lz.GPL_ID
	        , lz.REGION_NAME
	    	, lz.CHROMOSOME
	    	, lz.START_BP
...
ALTER FUNCTION tm_cz.i2b2_load_chrom_region(character varying, numeric) SET search_path=tm_cz, deapp;

ALTER FUNCTION tm_cz.i2b2_load_chrom_region(character varying, numeric) OWNER TO postgres;
